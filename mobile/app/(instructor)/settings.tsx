...
export default function InstructorSettingsScreen() {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      ...
      try {
        const profile = await instructorApi.getProfile(user.email);
  ...
  const handleUpdate = async () => {
    ...
    setLoading(true);
    try {
      const updated = await instructorApi.updateProfile(user.id, { 
        name, nic, phone, address, specialty, experience 
      });
          ...
          <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
            ...
            <Animated.View entering={FadeInDown.duration(700)} style={styles.header}>
              <View style={styles.headerRow}>
                <View style={[styles.headerIconBox, { backgroundColor: theme.primary + '18' }]}>
                  <Ionicons name="settings" size={22} color={theme.primary} />
                </View>
                <View style={styles.headerTextBlock}>
                  <ThemedText style={styles.title}>Instructor Profile</ThemedText>
                  <ThemedText style={styles.subtitle}>Manage your professional details</ThemedText>
                </View>
              </View>
              <View style={[styles.headerDivider, { backgroundColor: theme.primary + '20' }]} />
            </Animated.View>

            {/* Avatar Block */}
            <Animated.View entering={FadeInDown.delay(100).duration(700)} style={styles.avatarBlock}>
              <LinearGradient colors={[theme.primary, theme.secondary]} style={styles.avatarCircle}>
                <Ionicons name="person" size={38} color="#fff" />
              </LinearGradient>
              <View style={styles.avatarInfo}>
                <ThemedText style={styles.avatarName}>{user?.name || 'Instructor'}</ThemedText>
                <View style={styles.rolePill}>
                  <ThemedText style={styles.rolePillText}>INSTRUCTOR</ThemedText>
                </View>
              </View>
            </Animated.View>
            ...
            <Animated.View entering={FadeInDown.delay(300).duration(700)} style={{ marginTop: 20 }}>
              <View style={styles.sectionHead}>
            ...
            {/* Account */}
            <Animated.View entering={FadeInDown.delay(400).duration(700)} style={{ marginTop: 20 }}>
              <View style={styles.sectionHead}>
...
const styles = StyleSheet.create({
  ...
  headerIconBox: { width: 44, height: 44, borderRadius: 14, justifyContent: 'center', alignItems: 'center', marginRight: 14 },
  ...
  avatarBlock: { flexDirection: 'row', alignItems: 'center', marginBottom: 28, paddingBottom: 22, borderBottomWidth: 1, borderBottomColor: 'rgba(0,0,0,0.06)' },
  avatarCircle: { width: 72, height: 72, borderRadius: 22, justifyContent: 'center', alignItems: 'center', elevation: 6, shadowColor: '#000', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.2, shadowRadius: 8 },
  avatarInfo: { marginLeft: 16 },
  avatarName: { fontSize: 19, fontWeight: '900' },
  rolePill: { backgroundColor: 'rgba(59,130,246,0.12)', paddingHorizontal: 10, paddingVertical: 4, borderRadius: 8, marginTop: 5 },
  rolePillText: { fontSize: 11, fontWeight: '800', color: '#3B82F6', letterSpacing: 0.5 },
  ...
  readOnly: { padding: 18, borderRadius: 16, backgroundColor: 'rgba(0,0,0,0.04)', marginBottom: 36 },
  ...
  saveBtn: { height: 56, borderRadius: 18, flexDirection: 'row', justifyContent: 'center', alignItems: 'center', marginBottom: 16, elevation: 4 },
  saveBtnText: { color: '#fff', fontWeight: '800', fontSize: 16 },
  logoutBtn: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', height: 52, borderRadius: 16, borderWidth: 1.5, borderColor: 'rgba(239,68,68,0.2)' },
